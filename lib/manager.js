    ...
    validate(order) {
        return __awaiter(this, void 0, void 0, function* () {
            if (isUndefined(order.buyer))
                return new index_1.OrderError(order, `[Tradable] Error: validation error, buyer is required`);
            if (isUndefined(order.selledBy))
                return new index_1.OrderError(order, `[Tradable] Error: validation error, selledBy is required`);
            if (isUndefined(order.expirationDate))
                return new index_1.OrderError(order, `[Tradable] Error: validation error, expirationDate is required`);
            if (isUndefined(order.currency))
                return new index_1.OrderError(order, `[Tradable] Error: validation error, currency is required`);
            if (isUndefined(order.amount))
                return new index_1.OrderError(order, `[Tradable] Error: validation error, amount is required`);
            if (!this.validateMinimumAmount(order))
                return new index_1.OrderError(order, `[Tradable] Error: validation error, Amount is below the lower limit.`);
            try {
                const items = yield order.items.get(this._OrderItem);
                if (!this.validateCurrency(order, items))
                    return new index_1.OrderError(order, `[Tradable] Error: validation error, Currency of OrderItem does not match Currency of Order.`);
                if (!this.validateAmount(order, items))
                    return new index_1.OrderError(order, `[Tradable] Error: validation error, The sum of OrderItem does not match Amount of Order.`);
            }
            catch (error) {
                return error;
            }
                        ...
                        for (const item of items) {
                            ...
                            switch (sku.inventory.type) {
                                case index_1.StockType.finite: {
                                    ...
                                    if (remaining < 0) {
                                        const error = new index_1.OrderError(order, `[Failure] ORDER/${order.id}, [StockType ${sku.inventory.type}] SKU/${sku.id} is out of stock.`);
                                        reject(error);
                                    }
    ...
    pay(order, options, batch) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(order.status === index_1.OrderStatus.received || order.status === index_1.OrderStatus.waitingForPayment)) {
                throw new index_1.OrderError(order, `[Failure] pay ORDER/${order.id}, Order is not a payable status.`);
            }
            if (!options.customer && !options.source) {
                throw new index_1.OrderError(order, `[Failure] pay ORDER/${order.id}, PaymentOptions required customer or source`);
            }
            if (!options.vendorType) {
                throw new index_1.OrderError(order, `[Failure] pay ORDER/${order.id}, PaymentOptions required vendorType`);
            }
            if (!this.delegate) {
                throw new index_1.OrderError(order, `[Failure] pay ORDER/${order.id}, Manager required delegate`);
            }
            if (order.amount > 0) {
                try {
                    order.status = index_1.OrderStatus.paid;
                    ...
                    yield index_1.firestore.runTransaction((transaction) => __awaiter(this, void 0, void 0, function* () {
                        return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                            try {
                                ...
                                let _error = new index_1.OrderError(order, error.message, error.stack);
                                reject(_error);
                            }
                        }));
                    }));
    ...
    refund(order, options, batch) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(order.status === index_1.OrderStatus.paid || order.status === index_1.OrderStatus.transferred || order.status === index_1.OrderStatus.waitingForTransferrd)) {
                throw new index_1.OrderError(order, `[Failure] refund ORDER/${order.id}, Order is not a refundable status.`);
            }
            if (!options.vendorType) {
                throw new index_1.OrderError(order, `[Failure] refund ORDER/${order.id}, PaymentOptions required vendorType`);
            }
            if (!this.delegate) {
                throw new index_1.OrderError(order, `[Failure] refund ORDER/${order.id}, Manager required delegate`);
            }
            if (order.amount > 0) {
                try {
                    order.status = index_1.OrderStatus.refunded;
    ...
    transfer(order, options, batch) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!(order.status === index_1.OrderStatus.paid || order.status === index_1.OrderStatus.waitingForTransferrd)) {
                throw new index_1.OrderError(order, `[Failure] transfer ORDER/${order.id}, Order is not a transferable status.`);
            }
            if (!options.vendorType) {
                throw new index_1.OrderError(order, `[Failure] transfer ORDER/${order.id}, PaymentOptions required vendorType`);
            }
            if (!this.delegate) {
                throw new index_1.OrderError(order, `[Failure] transfer ORDER/${order.id}, Manager required delegate`);
            }
            if (order.amount > 0) {
                try {
                    order.status = index_1.OrderStatus.transferred;
