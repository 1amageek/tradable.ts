...
class Manager {
    ...
    async validate(order) {
        ...
        try {
            const items = await order.items.get(this._OrderItem);
    ...
    async refund(order, options, batch) {
        // Skip for refunded
        if (order.status === index_1.OrderStatus.refunded) {
            return;
        }
    ...
    async transfer(order, options, batch) {
        // Skip for 
        if (order.status === index_1.OrderStatus.transferred) {
            return;
        }
